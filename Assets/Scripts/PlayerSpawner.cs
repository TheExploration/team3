...
namespace FishNet.Component.Spawning
{
    ...
    public class PlayerSpawner : MonoBehaviour
    {
        ...
        private void SceneManager_OnClientLoadedStartScenes(NetworkConnection conn, bool asServer)
        {
            ...
            PlayerController pc = nob.GetComponent<PlayerController>();
            if (pc != null)
            {
                // Call the Server method on PlayerController to set the SyncVar Player ID
                pc.SetPlayerId(connectedPlayerCount); // This assumes PlayerController has `public void SetPlayerId(int id)` marked with [Server]
                Debug.Log(connectedPlayerCount);
            }
